...
STEP_INPUT_REFS_METADATA = "input_refs.json"
...
STEP_OUTPUT_REFS_METADATA = "output_refs.json"
STEP_ARGS = "args.pkl"
STEP_ARGS_REFS_METADATA = "args_refs.json"
...
WORKFLOW_INPUT_DICT = "workflow_input_dict.json"
...
class TaskInspectResult:
    ...
    # The refs in the output of a workflow task.
    ...
    def is_recoverable(self) -> bool:
        return (
            ...
            or (
                self.args_valid
                and self.upstream_dependencies is not None
                and self.upstream_data_dependencies is not None
...
class WorkflowStorage:
    ...
    def __init__(self, workflow_id: str):
        ...
        if SERVICE_SEP in workflow_id:
            # TODO(suquark): this is a temporary solution for workflow service.
            wid = workflow_id.split(SERVICE_SEP, maxsplit=1)[0]
            self._aux_storage = storage.get_client(os.path.join(WORKFLOW_ROOT, wid))
    ...
    def save_workflow_execution_state(
        ...
        for task_id, task in state.tasks.items():
            ...
            metadata = {
                **task.to_dict(),
                "upstream_dependencies": state.upstream_dependencies[task_id],
                "upstream_data_dependencies": state.upstream_data_dependencies[task_id],
            }
            ...
            self._storage.put(self._key_task_function_body(task_id), task.func_body)
            self._put(self._key_task_args(task_id), args_obj)

            # store with "task_id", because it is where we are going to
            # load the task output.
            object_refs = state.object_ref_tracker.get_task_outputs_metadata(task_id)
            actors = state.actor_tracker.get_task_outputs_metadata(task_id)
            self._put(
                self._key_task_input_refs_metadata(task_id),
                {"object_refs": object_refs, "actors": actors},
                is_json=True,
            )
    ...
    def save_task_output(
        self,
        task_id: TaskID,
        output_serialized: bytes,
        object_refs: List[ray.ObjectRef],
        actors: List[ray.actor.ActorHandle],
        *,
        exception: Optional[Exception],
        refs_hex_in_workflow_refs: Optional[Set[str]],
    ) -> None:
        ...
        Args:
            ...
            output_serialized: The returned object from a workflow task.
            exception: This task should throw exception.
        """
        if exception is None:
            # This workflow task returns an object.
            self._storage.put(self._key_task_output(task_id), output_serialized)
            # trigger actor checkpointing first
            actor_checkpoint_tasks = []
            for actor in actors:
                try:
                    actor_checkpoint_tasks.append(actor.checkpoint.remote())
                except Exception:
                    pass
            if refs_hex_in_workflow_refs is None:
                refs_hex_in_workflow_refs = set()
            for ref in object_refs:
                # skip checkpointing existing object refs
                if ref.hex() not in refs_hex_in_workflow_refs:
                    self.save_object_ref(ref)
            ray.wait(
                actor_checkpoint_tasks,
                num_returns=len(actor_checkpoint_tasks),
                fetch_local=False,
            )
            # NOTE: We always checkpoint the task output ref metadata as the last step,
            # so it marks the task checkpointing as committed.
            if not object_refs and not actors:
                # This is an optimization to checkpoint in the task instead of
                # the workflow executor. This helps reduce the overhead in the
                # workflow executor, especially under async checkpointing.
                self.save_task_output_ref_metadata(task_id, [], [])
        else:
            assert output_serialized is None
            self._put(self._key_task_exception(task_id), exception)

        # Finish checkpointing.
        # TODO(suquark): batching all tasks above.

    def save_task_args_ref_metadata(
        self, task_id: TaskID, object_refs: List[Dict], actors: List[Dict]
    ):
        """Save the metadata of the refs in the arguments of a task."""
        self._put(
            self._key_task_args_refs_metadata(task_id),
            {"object_refs": object_refs, "actors": actors},
            is_json=True,
        )

    def save_task_output_ref_metadata(
        self, task_id: TaskID, object_refs: List[Dict], actors: List[Dict]
    ):
        """Save the metadata of the refs in the output of a task."""
        self._put(
            self._key_task_output_refs_metadata(task_id),
            {"object_refs": object_refs, "actors": actors},
            is_json=True,
        )

    def load_task_func_body(self, task_id: TaskID) -> bytes:
        ...
        Returns:
            The binary of a callable function.
        """
        return self._storage.get(self._key_task_function_body(task_id))
    ...
    def save_object_ref(self, obj_ref: ray.ObjectRef) -> None:
        ...
        Returns:
            None
        """
        return self._save_object_ref(obj_ref.hex(), obj_ref)

    def object_ref_exists(self, object_id: str) -> bool:
        return self._exists(self._key_obj_id(object_id))
    ...
    def load_object_ref(self, object_id: str) -> ray.ObjectRef:
        ...
        Returns:
            The object ref.
        """
        return _put_obj_ref.remote(self._workflow_id, object_id)
    ...
    def inspect_output(self, task_id: TaskID) -> Optional[TaskID]:
        ...
        """
        # Workflow service does not store status
    ...
    def _inspect_task(self, task_id: TaskID) -> TaskInspectResult:
        ...
        try:
            ...
            result = TaskInspectResult(
                ...
                upstream_dependencies=metadata["upstream_dependencies"],
                upstream_data_dependencies=metadata["upstream_data_dependencies"],
        ...
        # does this task contain output checkpoint file?
        if STEP_OUTPUT in keys and STEP_OUTPUT_REFS_METADATA in keys:
            output_refs = self._get(self._key_task_output_refs_metadata(task_id), True)
            result.output_object_valid = True
            result.output_refs = output_refs
            return result
        # do we know where the output comes from?
        elif STEP_OUTPUTS_METADATA in keys:
            result.output_task_id = self._locate_output_task_id(task_id)
            return result
        else:
            return result
    ...
    def save_workflow_input_dict(self, input_dict: Dict):
        self._put(WORKFLOW_INPUT_DICT, input_dict, True)
    ...
    def _put(self, key: str, data: Any, is_json: bool = False) -> str:
        ...
        try:
            if not is_json:
                self._storage.put(key, cloudpickle.dumps(data))
            else:
    ...
    def _get_raw(self, key: str) -> Optional[bytes]:
        ...
        if v is None and self._aux_storage is not None:
            return self._aux_storage.get(key)
        return v
    ...
    def _scan_raw(self, prefix: str) -> List[str]:
        if self._aux_storage is None:
            return [p.base_name for p in self._storage.list(prefix)]
        try:
            return [p.base_name for p in self._storage.list(prefix)]
        except Exception:
            return [p.base_name for p in self._aux_storage.list(prefix)]

    def _scan(self, prefix: str, ignore_errors: bool = False) -> List[str]:
        try:
            return self._scan_raw(prefix)
        except Exception as e:
            if ignore_errors:
                return []
            raise e
    ...
    def _exists(self, key: str) -> bool:
        exist = self._storage.get_info(key) is not None
        if not exist and self._aux_storage is not None:
            return self._aux_storage.get_info(key) is not None
        return exist
    ...
    def _key_task_input_refs_metadata(self, task_id):
        return os.path.join(STEPS_DIR, task_id, STEP_INPUT_REFS_METADATA)

    def _key_task_output_refs_metadata(self, task_id):
        return os.path.join(STEPS_DIR, task_id, STEP_OUTPUT_REFS_METADATA)

    def _key_task_args_refs_metadata(self, task_id):
        return os.path.join(STEPS_DIR, task_id, STEP_ARGS_REFS_METADATA)
...
@ray.remote(num_cpus=0)
def _put_obj_ref(workflow_id: str, object_id: str):
    """
    ...
    return store._get(store._key_obj_id(object_id))
