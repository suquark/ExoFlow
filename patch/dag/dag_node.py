...
class DAGNode(DAGNodeBase):
    ...
    def apply_recursive(self, fn: "Callable[[DAGNode], T]", _cache: Dict = None) -> T:
        ...
        if type(self).__name__ == "InputNode":
            if "input" in _cache and self is not _cache["input"]:
    ...
    def _copy(
        ...
        instance._bound_upstream_nodes.extend(new_upstream_nodes)
        return instance

    def __lshift__(self, other: "Union[DAGNode, List[DAGNode]]"):
        if isinstance(other, DAGNode):
            self._bound_upstream_nodes.append(other)
        else:
            for n in other:
                self._bound_upstream_nodes.append(n)
        return other

    def __rshift__(self, other: "Union[DAGNode, List[DAGNode]]"):
        if isinstance(other, DAGNode):
            other._bound_upstream_nodes.append(self)
        else:
            for n in other:
                n._bound_upstream_nodes.append(self)
        return other
