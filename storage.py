...
def _check_exists_and_normalize(full_path: str) -> bool:
...
class KVClient:
    ...
    def __init__(self, fs: "pyarrow.fs.FileSystem", prefix: str):
        ...
        self._is_local_fs = isinstance(fs, LocalFileSystem)
        self._is_s3 = isinstance(fs, S3FileSystem)
    ...
    def put(self, path: str, value: bytes) -> None:
        ...
        if self._is_local_fs:
            exists = _check_exists_and_normalize(full_path)
            orig_path = full_path
            commit_path = full_path + ".commit"
            full_path = full_path + ".swap"
    ...
    def get(self, path: str) -> bytes:
        ...
        full_path = self._resolve_path(path)
        if self._is_local_fs:
            _check_exists_and_normalize(full_path)
        try:
            if self._is_s3:
                # patch arrow S3 read because it is 200x slower
                from smart_open import open as s_open

                with s_open("s3://" + full_path, "rb") as f:
                    return f.read()
            else:
    ...
    def list(
        ...
        if self._is_local_fs:
            if os.path.isdir(full_path):
                ...
                for f in files:
                    if not f.endswith(".swap") and not f.endswith(".commit"):
                        _check_exists_and_normalize(os.path.join(full_path, f))
