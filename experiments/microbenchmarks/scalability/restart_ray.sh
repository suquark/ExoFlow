...
for worker_ip in $RAY_WORKER_IPS; do
    ssh $worker_ip "source ~/anaconda3/bin/activate exoflow-dev; PYTHONPATH=$workdir ray start --address=$RAY_HEAD_IP:6379 --disable-usage-stats --object-manager-port=8076 --resources='{\"machine\": 1, \"controller\": 1}' --storage=$storage" &
